...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Failed to create manager' }));
      return NextResponse.json({ error: errorData.message || 'Failed to create manager' }, { status: response.status });
    }
